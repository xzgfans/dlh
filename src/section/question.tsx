...
const Question = () => {
  const {trys, setTrys, fenjus, setFenjus, answers, setAnswers, checkedItems, setYuanwen} = useContext(AppContext)
  useEffect(() => {
    nextQ()
  }, []);
  ...
  const hint = () => {
    setTrys([answers[0]])
    setFenjus(fenjus.map((f:IFenJu)=>({fenju:f.fenju, selected: f.fenju == answers[0]? true: false})))
  }
  const show = () => {
    setTrys([...answers])
